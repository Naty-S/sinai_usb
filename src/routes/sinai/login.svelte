...
<script lang="ts">
  ...
  const login = async function () {
    ...
    const dst_verify = `https://secure.dst.usb.ve/proxyValidate?service=http%3A%2F%2Flocalhost:3000%2Fsinai%2Flogin&ticket=${cas_ticket}`;
...
{#if cas_login}
  <Modal
    ...
    align="center"
    is_active={cas_login}
    close={() => { cas_login = false; console.log($page.url.pathname); login()}}
  >
  user -> {$session.user}
  </Modal>
{/if}
