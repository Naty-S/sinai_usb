...
<script lang="ts">
  ...
  import type { EntityActivities, YearActivities as YearActivitiesT } from "$lib/interfaces/activities";
  ...
  let pagination: YearActivitiesT[];
  let page_acts: YearActivitiesT[];

  const show_search = async function(e: any) {
    
    searching = true;
    const res: EntityActivities | string = await handleSubmit(e);
    searching = false;

    if (typeof res !== "string") {

      activities = res;
      pagination = activities?.by_year;
      page_acts = pagination.slice(start_pagination, end_pagination);
  ...
  const show_prev = function () {
    ...
    end_pagination = pagination_size / 2;

    if (start_pagination > 0) {

      page_acts = pagination.slice(start_pagination, end_pagination);
      // pagination.flatMap(p => Object.values(p.kind_activities))
    };
  };
  ...
  const show_next = function () {
    ...
    if (end_pagination < pagination.length) {
...
{:else if activities}
  ...
  <div>
    {#key page_acts}
      {#each page_acts as year_activities}
        <YearActivities {year_activities}/>
      {/each}
    {/key}

    <Pagination array={pagination.length} start={start_pagination} end={end_pagination} {show_prev} {show_next} />
  </div>
{:else}
  <div class="ui card">
