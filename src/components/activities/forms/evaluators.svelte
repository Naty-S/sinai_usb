  ...
  <div class="field">
    {#each $form.informe_tecnico.evaluadores as evaluator, i}
      <div class="inline fields">
        ...
        />
        <button class="ui red button" on:click={() => remove_evaluator(i)}>
          Elminar
        </button>
      </div>
    {/each}

    <button class="ui blue button" on:click|preventDefault={add_evaluator}>
      Agregar
    </button>
    {#if $form.informe_tecnico.evaluadores.length > 0}      
      <button class="ui red button" on:click={() => $form.informe_tecnico.evaluadores = []}>
        Limpiar
      </button>
    {/if}
  </div>
